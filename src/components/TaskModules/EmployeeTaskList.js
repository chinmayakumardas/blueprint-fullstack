...
export default function EmployeeTaskList({ tasks, currentUser }) {
  ...
  const taskStats = tasks
    ? {
        total: tasks.filter((task) => (viewMode === 'personal' ? task.assignee === currentUser.name : task.teamLeadId === currentUser.teamLeadId)).length,
        planned: tasks.filter((task) => (viewMode === 'personal' ? task.assignee === currentUser.name : task.teamLeadId === currentUser.teamLeadId) && task.status === 'Planned').length,
        inProgress: tasks.filter((task) => (viewMode === 'personal' ? task.assignee === currentUser.name : task.teamLeadId === currentUser.teamLeadId) && task.status === 'In Progress').length,
        completed: tasks.filter((task) => (viewMode === 'personal' ? task.assignee === currentUser.name : task.teamLeadId === currentUser.teamLeadId) && task.status === 'Completed').length,
        highPriority: tasks.filter((task) => (viewMode === 'personal' ? task.assignee === currentUser.name : task.teamLeadId === currentUser.teamLeadId) && task.priority === 'High').length,
        mediumPriority: tasks.filter((task) => (viewMode === 'personal' ? task.assignee === currentUser.name : task.teamLeadId === currentUser.teamLeadId) && task.priority === 'Medium').length,
        lowPriority: tasks.filter((task) => (viewMode === 'personal' ? task.assignee === currentUser.name : task.teamLeadId === currentUser.teamLeadId) && task.priority === 'Low').length,
      }
  ...
  const filteredAndSortedTasks = () => {
    ...
    } else if (viewMode === 'team' && currentUser.teamLeadId) {
      filtered = filtered.filter((task) => task.teamLeadId === currentUser.teamLeadId);
    }
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-2xl sm:text-3xl font-bold text-green-800">
                {viewMode === 'personal' ? 'My Tasks' : 'Team Tasks'}
              </h1>
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto">
              {currentUser.teamLeadId && (
                <div className="flex items-center gap-2">
                  <Switch
                    ...
                    onCheckedChange={() => setViewMode(viewMode === 'personal' ? 'team' : 'personal')}
                    className="data-[state=checked]:bg-green-600"
                  />
                  <span className="text-green-700">Team View</span>
                </div>
              )}
              ...
              {(currentUser.role === ROLES.TEAM_LEAD || viewMode === 'team') && (
          ...
          <p className="text-green-600 mb-6 max-w-md mx-auto">
            {selectedStatus === 'all' && selectedPriority === 'all' && !searchTerm
              ? viewMode === 'personal'
                ...
                : 'No tasks have been assigned to your team.'
                ...
                <TableRow key={task.taskId} className="hover:bg-green-50">
                  ...
                  <TableCell>
                    <div className="flex gap-2">
                      ...
                      {(currentUser.role === ROLES.TEAM_LEAD && viewMode === 'team' && task.teamLeadId === currentUser.teamLeadId) && (
                        <>
                          <Button
                            variant="ghost"
                            size="icon"
                      ...
                      {viewMode === 'personal' && task.status !== 'Completed' && (
                        <Button
                          variant="outline"
                          size="sm"
